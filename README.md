...
| PC 快捷键 | A–D / 1–4 选择，回车确认或下一题，← / → 切换题目 |
...
```
MindGym/
├── index.html              # 主入口（单页应用，≈38 KB）
├── questions.js            # 题库数据（≈275 KB）
├── questions.json          # 题库 JSON 版（≈331 KB，可选）
├── parse_bank.py           # 题库解析脚本（docx → JSON + JS）
├── 两学题库-合成版.docx     # 原始题目文档（不上传托管）
└── README.md
```

**GitHub / Pages 部署时**：只需上传 `index.html` + `questions.js` 即可。
